...
#R code for IMC data analysis of antigens ON versus OFF on days 10 and 20
...
####DATA LOADING####
setwd("~/Library/CloudStorage/Dropbox/Hopkins/Independent study/Jaffee Lab/Manuscripts/Neutrophils in metastatic disease/Codes/Antigen_expression_comparison") ## Set working directory
work<-getwd()
...
daylevels=c("10","20")  #Days 10 and 20 post-intraportal injection of tumor cells
doxlevels=c("On","Off") #Dox ON = Antigens OFF; Dox OFF = Antigens ON
...
ggdf$Day <- factor(output$meta_data$Day[match(ggdf$sample_id,output$meta_data$sample_id)], levels = daylevels)
ggdf$Dox <- factor(output$meta_data$Dox[match(ggdf$sample_id,output$meta_data$sample_id)], levels = doxlevels)
...
ggp<-ggplot(ggdf, aes(x = sample_id, y = cell_counts, fill = Dox)) + 
...
ggdf$Dox <- factor(output$meta_data$Dox[match(ggdf$sample_id,output$meta_data$sample_id)], levels = doxlevels)
ggdf$File_order <- factor(output$meta_data$File_order[match(ggdf$sample_id,output$meta_data$sample_id)], levels = fileorderlevels)
ggp<-ggplot(ggdf, aes(x = MDS1, y = MDS2, color = Dox, shape = Day)) +
  geom_point(size = 2.5) +
  theme_bw()+
...
##Clustering
output[(length(output)+1):(length(output)+3)] <- clusterfcs(fcs=output$fcs, numclusters=28, scaleoption = T) ## Set cluster number
...
clusterlevels=c(1:25) ## Set cluster number
colorassigned<-kovesi.rainbow_bgyrm_35_85_c69(length(unique(cluster_merging$new_cluster)))
...
clusterlevels=c("Neutrophil", "Myeloid_I", "Myeloid_II", "Myeloid_III", "Myeloid_IV", "Myeloid_V", "Kupffer",
                "CD8_T_I", "CD8_T_II", "CD8_T_III", "CD4_T_I", "CD4_T_II", "Treg", "CD3_T",  "B", "GZMB", "Stroma", "Endothelial_I", "Endothelial_II",
                "Hepatocyte_I", "Hepatocyte_II", "Tumor_I", "Tumor_II", "Tumor_III", "Tumor_IV")
...
plot_clustering_heatmap_wrapper2(fcs=output$fcs,
                                 ...
                                 subtype_markers = c("CD45", "S100A9", "LY6G", "CD68", "F480", "CD11C", "CLEC4F", "PDL1", "PD1", "CD86", "MHCII", "CD206",
                                                     "PDPN", "CD79A", "CD3e", "CD8A", "CD4", "FOXP3", "TOX2", "TCF1TCF7", "GZMB", "B220",
                                                     "ASMA", "VIM", "CD31", "PANCK", "ECAD", "ARG1", "GFP", "KI67", "CD10"),
                                color_clusters = colorassigned,
                                cell_clustering = factor(output$cell_clustering1m, levels=clusterlevels), 
                                fileName = 'Clusteringheatmap_merged.pdf') ###FIGURE S2A###
dev.off()

## Save output list
saveRDS(output, file="antigen_expression_comparison_backup_output.rds")
...
write.csv(densities, 'Results_densities.csv') ###FIGURE S2C###
...
ggdfd$Day <- factor(output$meta_data$Day[match(ggdfd$sample_id,output$meta_data$sample_id)], levels=daylevels)
ggdfd$Dox <- factor(output$meta_data$Dox[match(ggdfd$sample_id,output$meta_data$sample_id)], levels=doxlevels)

## DENSITY PLOTS
ggp2<-ggplot(ggdfd,aes(x=paste0("D",Day,"_",Dox),y=densities,fill=paste0("D",Day,"_",Dox)))+
...
pdf('Density_box.pdf',width=8,height=6) ###FIGURE S2C###
ggp2
dev.off()