...
#R code for IMC data analysis of CRS-207 and CXCR2 inhibition
...
returnfcs <- function(FDR_cutoff=.05,
  ...
  md$Listeria <- factor(md$Listeria)
  md$CXCR2 <- factor(md$CXCR2)
...
####DATA LOADING####
setwd("~/Library/CloudStorage/Dropbox/Hopkins/Independent study/Jaffee Lab/Manuscripts/Neutrophils in metastatic disease/Codes/CRS-207_CXCR2_inhibition") ## Set working directory
work<-getwd()
...
listerialevels=c("On","Off")
cxcr2levels=c("On","Off")
...
ggdf$Listeria <- factor(output$meta_data$Listeria[match(ggdf$sample_id,output$meta_data$sample_id)], levels = listerialevels)
ggdf$CXCR2 <- factor(output$meta_data$CXCR2[match(ggdf$sample_id,output$meta_data$sample_id)], levels = cxcr2levels)
...
ggp<-ggplot(ggdf, aes(x = sample_id, y = cell_counts, fill = Listeria)) + 
...
ggdf$CXCR2 <- factor(output$meta_data$CXCR2[match(ggdf$sample_id,output$meta_data$sample_id)], levels = cxcr2levels)
ggdf$File_order <- factor(output$meta_data$File_order[match(ggdf$sample_id,output$meta_data$sample_id)], levels = fileorderlevels)
ggp<-ggplot(ggdf, aes(x = MDS1, y = MDS2, color = Listeria, shape = CXCR2)) +
  geom_point(size = 2.5) +
  theme_bw()+
...
clusterlevels=c(1:24) ## Set cluster number
colorassigned<-kovesi.rainbow_bgyrm_35_85_c69(length(unique(cluster_merging$new_cluster)))
...
clusterlevels=c("Neutrophil_I", "Neutrophil_II", "Myeloid_I", "Myeloid_II", "Myeloid_III", "Myeloid_IV",
                "Myeloid_V", "Myeloid_VI", "Myeloid_VII", "Myeloid_VIII", "Kupffer",
                "CD8_T_I", "CD8_T_II", "CD4_T", "Treg", "CD3_T", "B", "GZMB",
                "Stroma", "Hepatocyte", "Tumor_I", "Tumor_II", "Tumor_III", "UA")
...
plot_clustering_heatmap_wrapper2(fcs=output$fcs,
                                 ...
                                 subtype_markers = c("CD45", "CD11b", "CD11c", "S100A9", "Ly6G", "Ly6C", "MPO", "iNOS", "Granzyme_B", "CD68", "F4_80", "CLEC4F", 
                                                     "PDL1", "PD1", "CD86", "MHCII", "CD206", "CD79a", "CD3e", "CD8a", "CD4", "Foxp3", "CD19", "Tox_Tox2", 
                                                     "TCF1_TCF7", "B220","aSMA", "PanKeratin", "ECAD", "pSTAT1", "pSTAT3", "ARG1", "Ki67"),
                                color_clusters = colorassigned,
                                cell_clustering = factor(output$cell_clustering1m, levels=clusterlevels), 
                                fileName = 'Clusteringheatmap_merged.pdf') ###FIGURE S12E###
dev.off()

## Save output list
saveRDS(output, file="CRS-207_CXCR2_inhibition_backup_output.rds")
...
write.csv(densities, 'Results_densities.csv') ###FIGURE S12F,G###
...
ggdfd$CXCR2 <- factor(output$meta_data$CXCR2[match(ggdfd$sample_id,output$meta_data$sample_id)], levels=cxcr2levels)
ggdfd$Listeria <- factor(output$meta_data$Listeria[match(ggdfd$sample_id,output$meta_data$sample_id)], levels=listerialevels)

## DENSITY PLOTS
ggp2<-ggplot(ggdfd,aes(x=paste0("CXCR2_",CXCR2,"_Listeria_",Listeria),y=densities,fill=paste0("CXCR2_",CXCR2,"_Listeria_",Listeria)))+
...
pdf('Density_box.pdf',width=8,height=6) ###FIGURE S12F,G###
ggp2
dev.off()